...
var findPaperAndWords = function(opt, cb) {
  ...
  var ep = new EventProxy();
	...
	findPaper(opt, function(err, audioPaper) {
...
exports.find = function(opt, cb, next) {
    ...
    findPaperAndWords(opt, function(err, audioPaper, wordIDs) {
        if(err) return next(err);
...
function findSiteByAudioPaper(audioPaper, next) {
  ...
  findSites({query:{paper_id:{$in:paperIDs}}, order:{paper_id:'ASC'}}, function(err, sites) {
    if(err) return next(err);
		makeupAudioPaperWithSite(sites, audioPaper);
		next()
  });
};
...
var find = exports.find = function(opt, cb, next) {
  var ep = new EventProxy();
  ...
  findPaperAndWords(opt, function(err, audioPaper, wordIDs) {
    if(err) return next(err);
	    ...
	    exports.findWordByIDs(wordIDs, ep, next);
			findSiteByAudioPaper(audioPaper, function(err) {
				if(err) return next(err);
...
exports.publicTimeLine = function(req, res, next) {
	var timeline = req.headers['lastModified']|| getNow();
	var opt = {
		query: {
			share: 100,
			site_id: {IS:'NOT NULL'},
